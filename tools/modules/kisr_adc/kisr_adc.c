...
#define GPIO_FOR_RX_SIGNAL    25
#define DEV_NAME              "gpio_ui1"
...
static irqreturn_t rx_isr(int irq, void *data)
{
    ...
    if (pWrite == pRead)
    {
        ...
        if ( wasOverflow == 0 )
        {
            printk(KERN_ERR "KISR ADC - Buffer Overflow - IRQ will be missed");
            wasOverflow = 1;
        }
    }
    else
                          ...
                          size_t count, loff_t *pos)
{
    ...
    if ( _error_count != 0 )
    {
        printk(KERN_ERR "KISR ADC - Error writing to char device");
        return -EFAULT;
    }
    pRead = (pRead + 1) & (BUFFER_SZ-1);
...
static int __init kisr_init(void)
{
    ...
    ret = request_irq(rx_irqs[0], rx_isr, IRQF_TRIGGER_FALLING, "rfrpi#rx", NULL);
...
MODULE_DESCRIPTION("Linux Kernel Module for gpio falling interrupts");
